...
class TestSyncNationDistrictsDuplicateGuard:
    """Regression coverage for the "Twinborn Elitria" bug: this nation's
    forge/ranch districts were only "legally" adjacent to a duplicate
    citadel that had somehow ended up on two different map tiles under the
    same id — a duplicate created by exactly this Direction-2 code path
    placing a second copy of an id that was already on some tile it just
    didn't currently see as owned by the nation. This never creates a
    second placement for an id that already exists anywhere on the map."""

    def test_district_id_already_on_the_map_elsewhere_is_not_duplicated(self, test_db):
        nation = {
            "_id": ObjectId(), "name": "Test Nation", "government_type": "Standard",
            "districts": [{"_id": "existing1", "def_key": "farm", "node": "", "upgrades": []}],
        }
        test_db["district_defs"].insert_one({
            "key": "farm", "display_name": "Farm", "tile_requirement": "land", "modifiers": [],
        })
        # This district's real tile — owned by someone else (or otherwise
        # excluded from this nation's own tiles_with_district/owned_tiles),
        ...
        with _patched_mongo(test_db):
            report = adh.sync_nation_districts(nation, dry_run=False, tiles_with_district=[], owned_tiles=[])

        assert report["placed_on_map"] == []
        assert report["unplaceable"] == []
        assert report["skipped_duplicate_elsewhere"] == [{"id": "existing1", "def_key": "farm"}]
        # No second tile was created for this id.
        assert test_db["hex_map_tiles"].count_documents({"district.id": "existing1"}) == 1

    def test_world_district_ids_param_is_honored_without_a_query(self, test_db):
        """Explicit world_district_ids (the batched multi-nation path used by
        the admin sync tools and reconcile_map_and_ai_nations.py) is honored
        the same way as the self-fetch default."""
        nation = {
            "_id": ObjectId(), "name": "Test Nation", "government_type": "Standard",
            "districts": [{"_id": "existing1", "def_key": "farm", "node": "", "upgrades": []}],
        }
        ...
        with _patched_mongo(test_db):
            report = adh.sync_nation_districts(
                nation, dry_run=False, tiles_with_district=[], owned_tiles=[],
                world_district_ids={"existing1"},
            )
